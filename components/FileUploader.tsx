...
export default function FileUploader<T extends FieldValues>({
    ...
    const handleRemove = (e: React.MouseEvent, onChange: (...event: any[]) => void) => {
        e.stopPropagation();
        setFileName(null);
        onChange(undefined);
        if (inputRef.current) {
    ...
    return (
        <FormField
            control={control}
            name={name}
            render={({ field: { value, onChange, ref: fieldRef, ...rest } }) => {
                // If the field value was reset outside, update local state
                if (!value && fileName) {
                    setFileName(null);
                }
                                ...
                                {fileName ? (
                                    <div className="flex flex-col items-center justify-center gap-2">
                                        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-md shadow-sm">
                                            <span className="text-[#663820] font-medium truncate max-w-[200px]">{fileName}</span>
                                            {!disabled && (
                                                <button
                                                    type="button"
