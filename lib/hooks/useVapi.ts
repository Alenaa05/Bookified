...
export function useVapi(book: IBook) {
    ...
    useEffect(() => {
        const sdk = getVapi();
        if (!sdk) {
            // Display a developer warning on the UI when Vapi credentials are not yet set up
            setLimitError('NEXT_PUBLIC_VAPI_API_KEY is not defined in your .env.local file.');
            return;
        }
        ...
        Object.entries(handlers).forEach(([event, handler]) => {
            sdk.on(event as keyof typeof handlers, handler as () => void);
        });
        ...
        return () => {
            ...
            // Cleanup handlers
            Object.entries(handlers).forEach(([event, handler]) => {
                sdk.off(event as keyof typeof handlers, handler as () => void);
            });
    ...
    const start = useCallback(async () => {
        ...
        const sdk = getVapi();
        if (!sdk || !ASSISTANT_ID) {
            setLimitError('Vapi API Key or Assistant ID is not set in environment variables. Please check your .env.local file.');
            return;
        }
